...
class notification_event_listener implements EventSubscriberInterface
{
	...
	static public function getSubscribedEvents()
	{
		return [
			...
			// cBB Reactions extension
			'reactions.add_remove_after'		=> 'handle_reaction_add_remove',
		];
	}
	...
	public function handle_reaction_add_remove($event)
	{
		$notification_type_config_name = 'discord_notification_type_reaction';
		...
		{
			if ($event['mode'] === 'add_reaction')
			{
				$this->notification_service->enrich_event_from_post_id($event);
				$webhook_url = $this->notification_service->get_forum_notification_url($event['forum_id']);
				if ($webhook_url)
				{
					$this->notify_post_reaction($event, $webhook_url);
				}
			}
		}
...
//reaction_id = {int} 2
	...
	private function notify_post_reaction($data, $webhook_url)
	{
		// Constant properties for this notification type
		$notification_type_config_name = 'discord_notification_type_reaction';
		$color = self::COLOR_BRIGHT_PURPLE;
		$emoji = self::EMOJI_REACT;

		// Construct the notification message using the argument data.

		$user_name = $this->notification_service->query_user_name($data['user_id']);
		$user_link = $this->generate_user_link($data['user_id'], $user_name);
		$post_link = $this->generate_post_link($data['topic_id'], $data['post_id'], $this->language->lang('POST'));

		$message = sprintf($this->language->lang('REACTION_POST'),
			$emoji, $user_link, $post_link
		);
